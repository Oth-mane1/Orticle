...
supprimer.forEach(element => {
    element.addEventListener('click', function (e) {
        const idArt = e.currentTarget.parentElement.parentElement.parentElement.parentElement.getAttribute('data-id')
        swal.fire({
            title: 'Suppression...',
            ...
            didOpen: () => {
                Swal.showLoading()
                fetch(`http://localhost:5000/app/article/delete/${idArt}`, {
                    method: 'DELETE'
                }).then((res) => {
                    if (res.status == 204) {
                        swal.fire({
